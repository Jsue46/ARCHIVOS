...
func GenerarReporteBitmapBloques(outputPath string, id string) string {
	...
	var bitmap []byte = make([]byte, blockBitmapSize)
	if err := Utils.LeerArchivo(file, &bitmap, int64(superblock.S_bm_block_start)); err != nil {
	...
	return fmt.Sprintf("Reporte bitmap de bloques generado exitosamente en: %s", outputPath)
}
